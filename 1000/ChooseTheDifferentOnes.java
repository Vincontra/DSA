...
import java.util.*;
import java.math.*;
// Now check if we can pick need (= k/2) from each
// If A has at least need distinct numbers among 1..k → onlyA + both >= need
// Similarly for B → onlyB + both >= need
    ...
    // ---------- Solve ----------
        static void solve() {
          ...
          for (int i=0;i<arr.length;i++){
            ...
            boolean check=false;
            for (int i=1;i<=k;i++){
                boolean isA= hm1.containsKey(i);
                boolean isB=hm2.containsKey(i);
                if (isA&&isB){
                    Both++;
                }
                else if (isA){
