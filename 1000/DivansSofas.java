...
public class DivansSofas{
    ...
    static void solve() {
        ...
        for (int i=0;i<n;i++) {
            l1.add(new Pair(arr[i],i));
        }

        l1.sort((p1,p2)->Long.compare(p2.vis, p1.vis));
        ...
        int start=1;
        for (int i=0;i<n;i++){
            brr[l1.get(i).idx+1]=start;
            ans+=2*Math.abs(start)*l1.get(i).vis;
            if (start<0){
                start=start*(-1)+1;
            }
            else{
