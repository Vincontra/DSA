...
public class LukeFoodie{
    ...
    static void solve() {
        ...
        long ans=0;
        // har baaar what we need to take care of the range right agar range ke andar hai to no
        // need to change the ans or count but need to adjust the range for the upcoming ones
        // if out of range increment the ans and take the new or fresh range
        long minR=arr[0]-x;
        long maxR=arr[0]+x;
        for (int i=1;i<n;i++) {
            long currMin=(arr[i]-x);
            long currMax=(arr[i]+x);
            minR=Math.max(minR,currMin);
            maxR=Math.min(maxR,currMax);
            if (minR>maxR){
                ans++;
                minR=currMin;
                maxR=currMax;
