...
public class Monsters {
    ...
    static void solve() {
        ...
        for (int  i=0;i<n;i++){
            ...
            arr[i]=arr[i]%k;
            if (arr[i]==0){
                arr[i]=k;  // to handle the full damage ye bhul gaya tha isliye 2 baar galat
            }
        }
        // abhi we need to sort this val with index matlab decending order me acc to values
        // fir jo bhi indices honge that will be our ans
        ArrayList<Pair>l1=new ArrayList<>();
        for (int i=0;i<n;i++){
            l1.add(new Pair(arr[i],i+1));
        }

        l1.sort((p1, p2) -> {
            if (p1.val == p2.val) {
                return Long.compare(p1.index, p2.index); // agar same then index smaller chahiye
            }
            return Long.compare(p2.val, p1.val);  // decending so val badhi chahiye
        });

        for (int i=0;i<l1.size();i++){
            out.print((l1.get(i).index) + " ");
        }
        out.println();
    }
    static class Pair{
