...
public class Raspberries{
    ...
    static void solve() {
        ...
        if (k==2){
            ...
            for (int x : arr) {
                if (x % 4 == 0) hasDiv4 = true;
                if (x % 2 == 0) cntEven++;
            }
            if (hasDiv4 || cntEven >= 2) {
                out.println(0);
                return;
            }
            int bestToDiv4 = 4;
            for (int x : arr) {
                int cost = (4 - (x % 4)) % 4;
                if (cost > 0){
                    bestToDiv4 = Math.min(bestToDiv4, cost);
                }
            }
            int makeTwoEvens = (cntEven == 1 ? 1 : 2);
            out.println(Math.min(makeTwoEvens,bestToDiv4));
        }
