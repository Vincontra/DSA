...
//For any positive integer sequence a
//        , we always have gcd(a)≤min(a)
//        . Let x=min(a)
//        , then x
//        must be placed on the min([a1,a2,…,ai])
//        side. Thus, we can conclude that:
//
//        min([a1,a2,…,ai])=gcd([ai+1,ai+2,…,an])=x
//
//        Now, we need to select some numbers, excluding one occurrence of x
//        , to be placed on the gcd([ai+1,ai+2,…,an])
//        side, ensuring that gcd([ai+1,ai+2,…,an])=x
//        .
//
//        A greedy strategy is to place all multiples of x
//        on the gcd
//        side after removing one occurrence of x
//        . This minimizes gcd
//        while ensuring it remains a multiple of x
//        .
//
//        Thus, we only need to check whether the gcd
//        of the remaining multiples of x
//        (after removing one occurrence of x
//        ) is still equal to x
//        .
//
//        Time complexity: O(n+loga)
//        or O(nloga)
//        (depending on which gcd
//        implementation you use) per test case.
...
public class MINGCD{
    ...
    static void solve(){
        ...
        if (min==1){
            ...
            if (cnt>1){
                long gcd=gcd(l1.get(0),l1.get(1));
                for (int i=2;i<l1.size();i++){
                    gcd=gcd(gcd,l1.get(i));
                }
                if (gcd==min){
                    out.println("Yes");
                }else{
