...
public class MakeitEqual{
    ...
    static void solve() {
        ...
        for (long i=0;i<n;i++){
            arr[(int)i]=fr.nextLong();
        }
        for (long i=0;i<n;i++){
            brr[(int)i]=fr.nextLong();
        }
        ...
        // those who are already same we need to remove them first
        for (long key:hm1.keySet()){
            long cnt1=hm1.get(key);
            long cnt2=hm2.getOrDefault(key,0L);
            long min=Math.min(cnt2,cnt1);
            hm1.put(key,cnt1-min);
            if (hm2.containsKey(key)){
                hm2.put(key,cnt2-min);
            }
        }
        boolean check=false;
        for (long key:hm1.keySet()){
            if (key==0){
                if (hm1.get(key)!=hm2.getOrDefault(key,0L)){
                    check=true;
                    break;
                }
            }else{
                if (hm1.get(key)!=hm2.getOrDefault(k-key,0L)){
                    check=true;
                    break;
                }
        ...
        for (long key:hm2.keySet()){
            if (key==0){
                if (hm2.get(key)!=hm1.getOrDefault(key,0L)){
                    check=true;
                    break;
                }
            }else{
                if (hm2.get(key)!=hm1.getOrDefault(k-key,0L)){
                    check=true;
                    break;
                }
            }
