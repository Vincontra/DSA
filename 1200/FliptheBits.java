...
//        int cnt0=0;
//        int cnt1=0;
//        // my logic is simply to check at each index is it unmatched; once i get the matched part i will check the cnt of 1 and 0 if not equal return false
//        // if equal reset the cnt1=0; cnt0=0; and move forward and outside the loop we also have to check if we dont find the matched part at n-1 th index
//        for (int i=0;i<n;i++){
...
//        out.println("Yes");
        ...
        // now by this logic if there is chance in future while going from left to right i may miss the chance to flip it so we need to go by prefix cnt
        int pfx[]=new int[n];
        int cnt1=0;
        int cnt0=0;
        for (int i=0;i<n;i++){
        ...
        for (int i = n - 1; i >= 0; i--){
            char ca = a.charAt(i);
            if (flipped){
                ca = (ca == '1' ? '0' : '1'); //flips          //000111
                                                              // 110100
            }

            if (ca != b.charAt(i)) {
                // Need to flip prefix [0..i], so check balance
                if (pfx[i] != 0) {
                    ok = false;
                    break;
                }
