...
public class MakeThemEqual{
    ...
    static void solve() {
       ...
       if (check){
           for (int x=1;x<=n;x++){        // c='a' and curr=acac   so prev i was checking the last index only so it is given 2 oprs as answer but if we take// x=2 it can convert both c to a in single opr
               boolean ok=true;
               for (int j=x;j<=n;j+=x){
                   if (curr.charAt(j-1)!=c){
                       ok=false;
                       break;
                   }
           ...
           out.println(0);  // if all are equal
       }
    }
