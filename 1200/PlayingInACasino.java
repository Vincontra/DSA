...
public class PlayingInACasino{
    ...
    static void solve() {
      ...
      long sum=0;
        // we just have to add the elements from each col to arraylist and then sort the arraylist and for each num
        // we have to check the how many numbers are greater than the current number and then just multiplication just like
        // this:
      for (int j=0;j<m;j++){
          ...
          for (int k=0;k<l1.size();k++){
              long currnum=l1.get(k);
              int cntless=k;
              int cntgreater=l1.size()-k-1;
              sum+=(-1)*(currnum)*(cntless)+currnum*cntgreater;
          }
      }
