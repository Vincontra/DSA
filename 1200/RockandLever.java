...
public class RockandLever{
    ...
    static void solve() {
        ...
        for (int i=0;i<n;i++){
            long val=arr[i];
            int setBit=HighestSetBit(val);
            if (!hm.containsKey(setBit)){
                hm.put(setBit,new ArrayList<>());
                hm.get(setBit).add(val);
            }else{
                hm.get(setBit).add(val);
            }
        }
    ...
    static int HighestSetBit(long n){
