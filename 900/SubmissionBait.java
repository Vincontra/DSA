...
//        }
        ...
        // i was wrong about this as this greddy wont work
        // picking up always make winner Alice as
        // see the case 1 2 2
        // so if alice pick 2
        // bob will pick 2
        // alice has no move that means alice lose
        // but if alice starts with 1
        // then bob picks 2
        // then alice pick 2
        // now bob has no move so alice wins

        // so to make alice winner i feel
        // if there is a number which is largest with odd frequnecy
        // then on;y alice will win
        int arr[]=new int[n+1];
        for (int i=0;i<n;i++){
            int val=fr.nextInt();
            arr[val]++;
