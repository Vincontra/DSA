        ...
        while (true){
            if (p<=k){
                if (m>=arr[p-1]){
                    m-=arr[p-1];
                    ans++;
                    // p-1 se age sab ek pos se aage khiskana padega and win wale ko last me
                    int i=p-1;
                    int val=arr[p-1];
                    for (int j=i;j<=n-2;j++){
                        arr[j]=arr[j+1];
                    ...
                    // energy alredy khatam
                    break;
                }

            }else{
                // abhi optimal hoga ki min niklate from first k so that win wale ko max times choose krne ka mauka mile
                ...
                if (m>=min){
                    m-=min;
                    for (int j=minIdx;j<=n-2;j++){
                        arr[j]=arr[j+1];
                    }
                    arr[n-1]=min;
                    p--;  // win wala ek se picke khiska diya
