...
public class D{
    ...
    static void solve() {
       ...
       long pfx[]=new long[(int)n];
       pfx[0]=arr[0];
       for (int i=1;i<n;i++){
           pfx[i]=pfx[i-1]+arr[i];
       }

        long t[][]=new long[(int)n+1][(int)n+1];
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if(i>=j){
                    t[i][j]=0;
        ...
        System.out.println(func(t,arr,1,n-1,pfx,K));
    }
    public static long func(long t[][],long arr[],long i,long j,long pfx[],Long K){
        if (t[(int)i][(int)j]!=-1) return t[(int)i][(int)j];
        if (i>j) return 0;
        long ans=0;
        for(long k=i;k<j;k++){
            long val=0;
            if ((pfx[(int)j]-pfx[(int)i-1]!=K)){
                val++;
            }else{
                continue;
            }
            ans=val;
            long temp=func(t,arr,i,k,pfx,K)+func(t,arr,k+1,j,pfx,K);
            ans+=temp;
        }
        return t[(int)i][(int)j]=ans;
    }
