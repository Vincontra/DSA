...
public class StaminaAndTasks{
    ...
    static void solve() {
        ...
        double ans = func(t, ci, pi, 0);
        System.out.println(ans);
    }


    public static double func(double t[],int ci[], int pi[], int i) {
        if (i==ci.length){
            return 0;
        }
        if (t[i]!=-1)return t[i];
        return t[i]=Math.max(func(t,ci,pi,i+1),ci[i]+(1-pi[i]/100.0)*func(t,ci,pi,i+1));
    }
