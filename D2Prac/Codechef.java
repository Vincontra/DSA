...
public class Codechef{
    ...
    // ---------- Solve ----------
    static void solve() {
        long l=fr.nextLong(),w=fr.nextLong(),r=fr.nextLong(),g=fr.nextLong(),b=fr.nextLong();
        if (can4(l,w,r,g,b)){
            out.println(4);
        }
        else if (can5(l,w,r,g,b)){
            out.println(5);
    ...
    public static boolean can4(long L, long W, long R, long G, long B) {
        for (int aL = 0; aL <= 2; aL++) {
            for (int bL = 0; bL <= 2 - aL; bL++) {
                int cL = 2 - aL - bL;
                for (int aW = 0; aW <= 2; aW++) {
                    for (int bW = 0; bW <= 2 - aW; bW++) {
                        int cW = 2 - aW - bW;
                        long rNeed = aL * L + aW * W;
                        long gNeed = bL * L + bW * W;
                        long bNeed = cL * L + cW * W;
                        if (rNeed == R && gNeed == G && bNeed == B){
                            return true;
                        }
                    }
                }
    ...
    public static boolean can5(long L, long W, long R, long G, long B) {
        // Try splitting one L-side or one W-side; remaining 3 sides are monochrome
        // splitType 0 -> split an L (remL=1, remW=2); 1 -> split a W (remL=2, remW=1)
        for (int splitType = 0; splitType < 2; splitType++) {
            long splitLen = (splitType == 0) ? L : W;
            int remL = (splitType == 0) ? 1 : 2;
            int remW = (splitType == 0) ? 2 : 1;
            for (int aL = 0; aL <= remL; aL++) {
                for (int bL = 0; bL <= remL - aL; bL++) {
                    int cL = remL - aL - bL;
                    for (int aW = 0; aW <= remW; aW++) {
                        for (int bW = 0; bW <= remW - aW; bW++) {
                            int cW = remW - aW - bW;
                            ...
                            // Pair (R,G) paints the split side
                            if (bNeed == B) {
                                long leftR = R - rNeed, leftG = G - gNeed;
                                if (leftR >= 0 && leftG >= 0 && leftR + leftG == splitLen) {
                                    return true;
                                }
                            }
                            // Pair (R,B)
                            if (gNeed == G) {
                                long leftR = R - rNeed, leftB = B - bNeed;
                                if (leftR >= 0 && leftB >= 0 && leftR + leftB == splitLen) {
                                    return true;
                                }
                            }
                            // Pair (G,B)
                            if (rNeed == R) {
                                long leftG = G - gNeed, leftB = B - bNeed;
                                if (leftG >= 0 && leftB >= 0 && leftG + leftB == splitLen){
                                    return true;
                                }
                            }
                        }
