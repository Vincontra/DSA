...
public class TheRoboticRush{
    ...
    static void solve() {
        ...
        String s = fr.next();
        ArrayList<Pair> befAft = new ArrayList<>();
        TreeSet<Pair> before = new TreeSet<>(
                (p1, p2) -> p1.x != p2.x ? p1.x - p2.x : p1.y - p2.y
        );
        TreeSet<Pair> after = new TreeSet<>(
                (p1, p2) -> p1.x != p2.x ? p1.x - p2.x : p1.y - p2.y
        );

        for (int i = 0; i < n; i++) {
            Integer up = b.higher(a[i]);
            Integer lo = b.lower(a[i]);

            int aft = up == null ? Integer.MAX_VALUE : up - a[i];
            int bef = lo == null ? Integer.MAX_VALUE : a[i] - lo;

            befAft.add(new Pair(bef, aft));
            before.add(new Pair(bef, i));
            after.add(new Pair(aft, i));
        }
        int val = 0;
        int ans = n;
        for (int i = 0; i < k; i++) {
            ...
            if(val > 0) {
                while (!after.isEmpty() && Math.abs(val) >= after.first().x) {
                    int idx = after.first().y;
                    after.pollFirst();
                    before.remove(new Pair(befAft.get(idx).x, idx));
                    ans--;
                }
            }
            else {
                while (!before.isEmpty() && Math.abs(val) >= before.first().x) {
                    int idx = before.first().y;
                    before.pollFirst();
                    after.remove(new Pair(befAft.get(idx).y, idx));
                    ans--;
                }
            }
            out.print(ans+" ");
