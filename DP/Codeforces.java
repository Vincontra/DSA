...
//        int n=fr.nextInt();
//        String s=fr.next();
        // 1-0-1-0 jab tak sab zero na ho jaye
        // agar cnt1==n then simply -1
...
//                cnt0++;
//            }else{
//                l1.add(i+1);  // yaha pr one ke index;
//            }
//        }
//        int cnt1=n-cnt0;
//        int st=0;
//        boolean one=true;
//        // pehle 1 ko choose then zero ko
//        boolean check=false;
//
//        while (true){
//            if (cnt1==n){
...
//    }
    ...
    static void solve() {
        ...
        preMin[0]=Long.MAX_VALUE;
        for (int i=0;i<n;i++) {
            pre[i+1]=pre[i]+a[i];
            preMin[i+1]=Math.min(preMin[i], a[i]);
        }
        sufMax[n]=Long.MIN_VALUE;
        for (int i=n-1;i>=0;i--){
            sufMax[i]=Math.max(sufMax[i + 1], a[i]);
        }
        // bs laga sakte on time
        long lo=1;
        long hi;
        long rds=(h+sum-1)/sum;
        hi=rds*(n+k);
        while (lo<hi){
            long mid=lo+(hi-lo)/2;
            long r=mid/(n+k);
            ...
            long bd=r*sum+pre[(int)p];
            long g=0;
            if (p>0&&p<n){
                if (sufMax[(int)p]!=Long.MIN_VALUE && preMin[(int)p]!=Long.MAX_VALUE) {
                    g=Math.max(0,sufMax[(int)p]-preMin[(int) p]);
                }
            }
            if (bd+g>=h){
                hi=mid;
