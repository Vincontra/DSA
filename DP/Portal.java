...
public class Portal{
    ...
    static void solve() {
        ...
        ArrayList<Integer> bichwala = new ArrayList<>();

        for (int i = 0; i < x; i++)l.add(arr[i]);
        for (int i = x; i < y; i++)bichwala.add(arr[i]);
        for (int i = y; i < arr.length;i++)l.add(arr[i]);
        ...
        int idx=0;
        for (int i=0;i<bichwala.size();i++) {
            if (min>bichwala.get(i)) {
                min=bichwala.get(i);
                idx=i;
            }
        }
        ...
        for (int i=idx;i<bichwala.size();i++) {  // arraylist me remove O(N) so n baar kra to n2 so may be tle isliye abhi aise likha hai
            r.add(bichwala.get(i));
        }
        for (int i=0;i<idx;i++) {
            r.add(bichwala.get(i));
        }
        ...
        for (int i1 = 0; i1 < l.size(); i1++) {
            if (bichwala.get(0)>l.get(i1)) {
                ans.add(l.get(i1));
            }
            else {
                ans.addAll(bichwala);
                for (int j = i1; j < l.size(); j++) {
                    ans.add(l.get(j));
                }
                check=true;
                break;
