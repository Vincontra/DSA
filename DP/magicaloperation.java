...
public class magicaloperation{
    ...
    static void solve() {
        ...
        for (int i=0;i<q;i++) {
            ...
            long x=fr.nextLong();
            long y=fr.nextLong();
            HashSet<Long>hs=func(x);
            HashMap<Integer,Long>hm2=new HashMap<>();
            for (long d:hs){
                if (!hm.containsKey(d)){
                    continue;
                }
                TreeSet<Integer>ts=hm.get(d);
                for (int j : ts.tailSet(idx, true)) {
                    hm2.putIfAbsent(j, d);
                }
            }
            for (int k : hm2.keySet()) {
                long purana=hm2.get(k);
                long naya=purana*y;
                a[k]=naya;
                hm.get(purana).remove(k);
                if (!hm.containsKey(naya)){
                    hm.put(naya, new TreeSet<>());
                }
                hm.get(naya).add(k);
            }
        }
    ...
    public static HashSet<Long> func(long x) {
