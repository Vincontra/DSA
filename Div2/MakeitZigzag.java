...
public class MakeitZigzag{
    ...
    static void solve() {
        ...
        // finished all the operations first
        // now only check for even indices (agar 1 based se dekhe to odd
        for (int i=0;i<n-1;i++) {
            if(i%2==0) { // 1 based me odd hai
                if(arr[i]>=arr[i+1]){
                    long change=arr[i]-arr[i+1]+1;
                    ans+=change;
                    arr[i]-=change;
                }
            }
