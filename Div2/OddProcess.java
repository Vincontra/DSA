...
//       int ev[]=new int[0];
//       if (even.size()>0){
//           Collections.sort(even,Collections.reverseOrder());  //{4,2}
//           ev=new int[even.size()];
//           ev[0]=even.get(0);   // now this will acts as a sfx sum as we have sorted that arraylist in reverse order
//           for (int i=1;i<even.size();i++){
//               ev[i]=ev[i-1]+even.get(i);
//           }
//       }
//       if (odd.size()>0){
//           Collections.sort(odd);
//           int od[]=new int[odd.size()];
//           od[0]=odd.get(0);
//           for (int i=1;i<odd.size();i++){
//               od[i]=od[i-1]+odd.get(i);
//           }
//       }
//       for (int i=1;i<=n;i++){
//           int idx=i;
...
//           }
//           else{
//               int sum=odd.get(odd.size()-1);
//               idx=idx-1; // -1 for the reason as we have added the biggest odd number
//               // now we have to check do we have enough even's if so directly add the sum of evens in descending order
//               if (even.size()>=idx){
//                   sum+=ev[idx-1];
//                   out.print(sum+" ");
//               }
//               else{
//                   // in this case we dont have enough even's so each time we will take 2 odd's that will be
//                   // added and gives us even which will be vanished and this will be done so far until
//                   // we dont have the enough even's
//                   while (idx>even.size()){
//                       idx-=2;
//                   }
//                   sum+=ev[idx-1];
...
//                        continue;
//                    }
//                    if (idx > 0) sum += ev[idx - 1];
//                    out.print(sum + " ");
//                } else {
//                    while (idx > even.size()) idx -= 2;
//
//                    if (idx < 0 || (i - idx) % 2 == 0) {
//                        out.print(0 + " ");
//                        continue;
...
//    }
    ...
    static void solve(){
        ...
        Collections.sort(odd);
        Collections.sort(even, Collections.reverseOrder());
        long[] prefeven = new long[even.size() + 1];
        prefeven[0] = 0;
        for (int i = 0; i < even.size(); i++) {
            prefeven[i + 1] = prefeven[i] + even.get(i);
        }
        int cntodd = odd.size();
        int cnteven = 0;
        for (int i = 0; i < n; i++) {
        ...
        for (int k = 1; k <= n; k++) {
            ...
            if ((odd1 & 1) == 1) {
                out.print(odd.get(odd.size() - 1) + prefeven[even1] + " ");
            }
            else {
                out.print("0 ");
            }
