...
public class XORFact{
    ...
    static void solve() {
        ...
        long dpFirst = 0;   // last removal was FIRST
        long dpSecond = Long.MIN_VALUE / 2; // invalid initially

        for (int i = 0; i < n; i++) {
            long newFirst = Math.max(dpFirst, dpSecond) + a[i];
            long newSecond = dpFirst - a[i];

            dpFirst = newFirst;
            dpSecond = newSecond;
        }

        // one child remains → max of both states
        out.println(Math.max(dpFirst, dpSecond));
    }
