...
public class CountingBits{
    ...
    static void solve() {
        long n = fr.nextLong();
        long Cnt = countTheBits(n);
        long ans = 0;
        for (int i = 0; i < Cnt; i++) {
            long CycleLen = 1L << (i + 1);   // 2^(i+1)
            long NoOfCycle = Math.floorDiv((n + 1), CycleLen);
            long Contribution = NoOfCycle * (1L << i);  // 2^i
            ans += Contribution;
            long rem = (n + 1) % CycleLen;
            ans += Math.max(0, rem - (1L << i));
        }
        out.println(ans);
    }

    public static long countTheBits(long n) {
        long cnt = 0;
        while (n != 0) {
            cnt++;
            n /= 2;
