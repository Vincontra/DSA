...
public class ExponentiationII{
    ...
    static void solve() {
        ...
        long b=fr.nextInt();
        long c=fr.nextInt();
        long bc=modPow1(b,c,MOD);
        long ans=modPow2(a,bc,MOD);
        out.println(ans);
    ...
    static long modPow1(long a, long b, long mod) {
        long res = 1;
        a %= (mod-1);
        while (b > 0) {
            if ((b & 1) == 1) res = (res * a) % (mod-1);
            a = (a * a) % (mod-1);
            b >>= 1;
        }
        return res;
    }
    static long modPow2(long a, long b, long mod) {
        long res = 1;
        a %=mod;
        while (b > 0) {
